"""用于特征裁剪（FP）状态的在策略评论家缓冲区模块。"""

import numpy as np
import torch

from baselines.utils.env import get_shape_from_obs_space
from baselines.utils.trans import _flatten, _ma_cast


class OnPolicyCriticBufferFP:
    """使用特征裁剪（FP）状态的在策略评论家缓冲区。

    当使用 FP 状态时，评论家对不同的智能体使用不同的全局状态作为
    输入。因此，与 OnPolicyCriticBufferEP 相比，
    OnPolicyCriticBufferFP 多了一个智能体数量的维度。
    """
    ...
    def compute_returns(
        ...
        Args:
            next_value: (np.ndarray) 最后一个回合步骤之后
                的价值预测。
            value_normalizer: (ValueNorm) 如果不为 None，
    ...
    def feed_forward_generator_critic(
        ...
        """使用 MLP 网络的评论家的训练数据生成器。

        Args:
            critic_num_mini_batch: (int) 评论家的小批次数量。
            mini_batch_size: (int) 评论家的小批次大小。
        """
        # 获取 episode_length、n_rollout_threads 和
        # mini_batch_size
        episode_length, n_rollout_threads, num_agents = (
            self.rewards.shape[0:3]
        )
        batch_size = (
        ...
        if mini_batch_size is None:
            assert batch_size >= critic_num_mini_batch, (
                ...
                f"* 步数 ({episode_length}) "
                f"* 智能体数 ({num_agents}) = "
                f"{n_rollout_threads * episode_length * num_agents} "
                f"需要大于或等于评论家小批次数 "
    ...
    def naive_recurrent_generator_critic(
        ...
        """使用 RNN 网络的评论家的训练数据生成器。

        此生成器不会将轨迹拆分为块，因此在训练中可能不如
        recurrent_generator_critic 高效。

        Args:
            critic_num_mini_batch: (int) 评论家的小批次
                数量。
        """
        # 获取 n_rollout_threads 和 num_envs_per_batch
        _, n_rollout_threads, num_agents = (
            self.rewards.shape[0:3]
        )
        batch_size = n_rollout_threads * num_agents
        assert batch_size >= critic_num_mini_batch, (
            f"PPO 要求进程数 ({n_rollout_threads})"
            f" * 智能体数 ({num_agents}) "
            f"大于或等于 PPO 小批次数 "
            f"({critic_num_mini_batch})。"
        )
        ...
        # (episode_length, n_rollout_threads
        #  * num_agents, *dim)
        share_obs = self.share_obs.reshape(
            -1, batch_size, *self.share_obs.shape[3:]
        )
        rnn_states_critic = (
            self.rnn_states_critic.reshape(
                -1,
                batch_size,
                *self.rnn_states_critic.shape[3:],
            )
        )
        value_preds = self.value_preds.reshape(
            -1, batch_size, 1
        )
    ...
    def recurrent_generator_critic(
        ...
        此生成器将轨迹拆分为长度为 data_chunk_length 的块，
        因此在训练中可能比
        ...
        # (episode_length, n_rollout_threads, num_agents)
        # 转置为
        # (n_rollout_threads, num_agents, episode_length)，
        # 然后将数据重塑为
        ...
        #      num_agents, *share_obs_shape)
        # --> (n_rollout_threads, num_agents,
        #      episode_length, *share_obs_shape)
        # --> (n_rollout_threads * num_agents
        #      * episode_length, *share_obs_shape)
