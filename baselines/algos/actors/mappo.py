...
class MAPPO(OnPolicyBase):
    ...
    def share_param_train(
        ...
        """
        使用小批量梯度下降执行参数共享 MAPPO 训练。

        参数:
            actor_buffer: 包含各智能体训练数据的缓冲区列表。
            advantages: 优势值数组。
            num_agents: 智能体数量。
            state_type: 状态类型。
        ...
        if state_type == "EP":
            ...
            for agent_id in range(num_agents):
                ...
                advantages_copy_list.append(
                    advantages_copy,
                )
            advantages_ori_tensor = np.array(
                advantages_ori_list,
            )
            advantages_copy_tensor = np.array(
                advantages_copy_list,
            )
            mean_advantages = np.nanmean(
                advantages_copy_tensor,
            )
            std_advantages = np.nanstd(
                advantages_copy_tensor,
            )
            normalized_advantages = (
                advantages_ori_tensor - mean_advantages
            ) / (std_advantages + 1e-5)
            advantages_list = []
            for agent_id in range(num_agents):
                advantages_list.append(
                    normalized_advantages[agent_id],
                )
        elif state_type == "FP":
            advantages_list = []
            for agent_id in range(num_agents):
        ...
        for _ in range(self.ppo_epoch):
            ...
            for _ in range(self.actor_num_mini_batch):
                ...
                for i in range(7):
                    batches[i] = np.concatenate(
                        batches[i], axis=0,
                    )
                if batches[7][0] is None:
                    batches[7] = None
                else:
                    batches[7] = np.concatenate(
                        batches[7], axis=0,
                    )
                (
                    policy_loss,
